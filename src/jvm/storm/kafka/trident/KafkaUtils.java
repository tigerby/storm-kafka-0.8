  ...
  public static Map emitPartitionBatchNew(TridentKafkaConfig config, SimpleConsumer consumer,
    ...
    if (lastMeta != null) {
      ...
      if (config.forceFromStart && !topologyInstanceId.equals(lastInstanceId)) {
        offset = getLastOffset(consumer, config.topic, partition.partition, config.startOffsetTime,
                               makeClientName(config.topic, partition.partition));
      } else {
      ...
      offset = getLastOffset(consumer, config.topic, partition.partition, startTime,
                             makeClientName(config.topic, partition.partition));
    }
    ...
    for (MessageAndOffset msg : fetchResponse.messageSet(config.topic, partition.partition)) {
      ...
      try {
        ...
        if (_partitions != null && _partitions.size() == _partitionToOffset.size()) {
          for (Map.Entry<GlobalPartitionId, Long> e : _partitionToOffset.entrySet()) {
            ...
            if (consumer == null) {
              LOG.warn(
                  "partitionToOffset contains partition not found in _connections. Stale partition data?");
              return null;
            }
            long
                latestTimeOffset =
                getLastOffset(consumer, _topic, partition.partition, OffsetRequest.LatestTime(),
